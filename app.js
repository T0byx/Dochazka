...
function render() {
  ...
  monthKeys.forEach(key => {
    ...
    if (wk !== currentWeek) {
      currentWeek = wk;
      // stats for this week (within currently shown month)
      const weekKeys = monthKeys.filter(k => isoWeek(k) === wk);
      ...
      wr.innerHTML = `<td colspan="5"><span class="wk-label">${wk}. týden</span><span class="wk-stats mono"><span>${fmtDur(wHours)}</span><span class="${saldoClass}">${fmtSaldo(wSaldo)}</span></span></td>`;
    ...
    if (e && e.type && e.type !== 'work') {
      ...
      const inTd = document.createElement('td'); inTd.className = 'mono nowrap';
      inTd.innerHTML = e?.checkIn ? e.checkIn : '<span class="cell-dash">–</span>';
      const outTd = document.createElement('td'); outTd.className = 'mono nowrap';
      outTd.innerHTML = e?.checkOut ? e.checkOut : '<span class="cell-dash">–</span>';
      const wh = workedHours(e);
      const workTd = document.createElement('td'); workTd.className = 'mono nowrap';
      workTd.innerHTML = wh !== null ? fmtDur(wh) : '<span class="cell-dash">–</span>';
      const sal = daySaldo(e);
      const salTd = document.createElement('td'); salTd.className = 'mono nowrap';
